...
const FloatingChat = () => {
  ...
  return (
    <>
      {isOpen ? (
        <div className="fixed bottom-20 right-8 z-50 w-[400px] max-h-[85vh] bg-gradient-to-br from-orange-50 via-pink-50 to-purple-50 rounded-2xl shadow-2xl border border-orange-200/50 flex flex-col overflow-hidden backdrop-blur-sm transform transition-all duration-300 ease-out">
          {/* Header */}
          <div className="bg-gradient-to-r from-purple-600 to-pink-600 text-white/90 backdrop-blur-sm border-b border-white/20 shadow-lg sticky top-0 z-10">
            <div className="max-w-4xl mx-auto px-6 py-4">
              <div className="flex items-center justify-between">
                <div>
                  <h1 className="text-xl font-bold bg-gradient-to-r from-yellow-300 to-white bg-clip-text text-transparent">
                    Chat Assistant
                  </h1>
              ...
              {messages.length === 0 ? (
                <div className="h-full flex items-center justify-center p-4">
                  <div className="text-center">
                    ...
                    <h3 className="text-lg font-semibold text-gray-800 mb-2">Welcome to Chat Assistant!</h3>
          ...
          <div className="bg-gradient-to-b from-purple-200 to-pink-300/90 backdrop-blur-sm border-t border-purple-400/60 p-4 sticky bottom-0">
            <div className="flex items-center space-x-3">
              <textarea
                ...
                className="flex-1 bg-gradient-to-r from-purple-200 to-pink-200 border border-purple-400 rounded-2xl px-4 py-3 resize-none text-base outline-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500 font-medium text-purple-900 placeholder-purple-500 min-h-[40px] max-h-32"
                rows={1}
                style={{ minHeight: '40px', fontSize: '1rem', lineHeight: '1.5' }}
              />
              <button
                onClick={handleSendMessage}
                disabled={!inputValue.trim()}
                className={`px-4 py-3 rounded-2xl font-bold text-base transition-all duration-300 ${
                  inputValue.trim()
            ...
            <p className="text-sm text-purple-800 mt-2 text-center font-medium bg-gradient-to-r from-purple-200 to-pink-200 px-4 py-2 rounded-full">
              Press Enter to send
            </p>
          </div>
        </div>
      ...
      <button
        onClick={toggleChat}
        className="fixed bottom-8 right-8 z-50 bg-gradient-to-r from-orange-500 to-pink-600 text-white p-5 rounded-full shadow-xl hover:shadow-2xl hover:from-orange-600 hover:to-pink-700 focus:outline-none focus:ring-4 focus:ring-orange-500/30 transition-all duration-300 group"
      ...
      >
        <div className="relative">
          ...
          <div className="absolute -top-1 -right-1 w-4 h-4 bg-green-400 rounded-full border-2 border-white group-hover:animate-pulse"></div>
        </div>
      </button>
    </>
  );
